    ...
    with col1:
        ...
        if tour_class =='Dubai in Tokyo':
            ...
            if n>=1:
                ...
                status = st.radio("choose payment method: ", ('MasterCard', 'Bank Transfer', 'Paystack', 'Flutterwave'))
                if(status == 'MasterCard'):
                    st.error('unavailable')
                elif(status == 'Bank Transfer'):
                    st.text('Account Name: Deen Tours Ltd.')
                    st.text('Bank: OLAYEMI PEOPLES BANK')
                    st.text('Account Number: 32xxxxxxxx')
